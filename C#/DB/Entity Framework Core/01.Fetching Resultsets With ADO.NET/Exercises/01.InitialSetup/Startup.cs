    ...
    public class Startup
    {
        static void Main()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                foreach (var sqlText in createStatement)
                {
                    ExecuteNonQuery(connection, sqlText);
                }

                Console.WriteLine("Tables for MinionsDB - Created");
                ...
                foreach (var sqlText in insertStatement)
                {
                    ExecuteNonQuery(connection, sqlText);
                }

                Console.WriteLine("Data for MinionsDB - Inseted");
            }
        }

        private static void ExecuteNonQuery(SqlConnection connection, string sqlText)
        {
